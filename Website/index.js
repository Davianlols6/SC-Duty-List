...
function showPersonalised() {
  ...
  $(week).each(function (i, v) {
    if (counter < 5) {
      ...
      var lastDate = new Date(
        Object.keys(jsonData["dates"])[
          Object.keys(jsonData["dates"]).length - 1
        ]
      );
      ...
      if (flagData[v] != null) {
        if (String($("#name").val()) === flagData[v].singapore_flag) {
          flagDuty =
            "<p><span class='fw-bold'></span><span class='badge bg-warning mb-1'>Flag Duty</span><br><span class='fw-bold'>Duty: </span><span id=''>Singapore Flag</span></p>";
        } else if (String($("#name").val()) === flagData[v].school_flag) {
          flagDuty =
            "<p><span class='fw-bold'></span><span class='badge bg-warning mb-1'>Flag Duty</span><br><span class='fw-bold'>Duty: </span><span id=''>School Flag</span></p>";
        }
      }
      ...
      if (
        ...
        a =
          "<div id='" +
          perDict[i].id +
          "' class='bg-black container shadow pt-3 pb-3 mb-3 bg-white rounded-3'><h4>" +
          days[b.getDay()] +
          ", " +
          String(b.getDate()) +
          " " +
          months[b.getMonth()] +
          "</h4><p><span class='fw-bold'></span>No data is available for this day</span></p></div>";
        ...
        a =
          ...
          recessBadge[
            nameData[String($("#name").val())][String(jsonData["dates"][v])][
              "recess"
            ]
          ] +
          "<br><span class='fw-bold'>Duty: </span><span id=''>" +
          dutyTrans[
            nameData[String($("#name").val())][String(jsonData["dates"][v])][
              "duty"
            ]
        ...
        var c =
          ...
          "</h4>" +
          flagDuty +
          "<p><span class='fw-bold'></span><span class='badge bg-warning mb-1'>Morning Duty</span><br><span class='fw-bold'>Duty: </span><span id=''>" +
          dutyTrans[
            nameData[String($("#name").val())]["morning_duty"]["duty"]
          ] +
          "</span></p><p><span class='fw-bold'></span>" +
          recessBadge[
            nameData[String($("#name").val())][String(jsonData["dates"][v])][
              "recess"
            ]
  ...
  $(
    "<div id='date2' class='text-center'><h4>Hello " +
      String($("#name").val()) +
      "!</h4><p>Here is your duty for the week you selected</p></div>"
  ).insertBefore("#date");
  $("#personalised").show();
  $(["#date", "#date2", "#day_1", "#day_2", "#day_3", "#day_4", "#day_5"]).each(
    function (i, v) {
      $(v).hide();
    }
  );
...
$.each(
  ["#weekend", "#date", "overview", "#version", "#footer"],
  function (i, v) {
    $(v).hide();
  }
);
...
$.getJSON("Data/data.json?time=" + new Date().getTime(), function (data) {
  var dt = new Date();
  ...
  $("#start").attr(
    "max",
    Object.keys(data["dates"])[Object.keys(data["dates"]).length - 1]
  );
...
function getFlagsData() {
  $.getJSON("Data/flag.json?time=" + new Date().getTime(), function (data) {
    flagData = data;
    getNameData();
  });
}
...
function getNameData() {
  $.getJSON("Data/names.json?time=" + new Date().getTime(), function (data) {
    nameData = data;
...
function recessTime(period) {
  var icon =
    "<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-record-circle mx-1 icon-blink' style='fill: #ff0000' viewBox='0 0 16 16'><path d='M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z' /><path d='M11 8a3 3 0 1 1-6 0 3 3 0 0 1 6 0z' /></svg>";
...
function update() {
  ...
  var firstDate = new Date(Object.keys(jsonData["dates"])[0]);
  var lastDate = new Date(
    Object.keys(jsonData["dates"])[Object.keys(jsonData["dates"]).length - 1]
  );
  if (
    dd.getDay() === 6 ||
    dd.getDay() === 0 ||
    jsonData["dates"][String($("#start").val())] === 11 ||
    dd < firstDate ||
    dd > lastDate
  ) {
    $("#date").hide();
  ...
  function updateMorningDuty() {
    var morningDutyData =
      jsonData["constants"][
        jsonData["presets"][jsonData["dates"][String($("#start").val())]][
          "constant_morning_duty"
        ]
      ];
    $("#morning_main_gate").text(listToString(morningDutyData["main_gate"]));
    $("#morning_late_coming").text(
      listToString(morningDutyData["late_coming"])
    );
    $("#morning_canteen").text(listToString(morningDutyData["canteen"]));
    $("#morning_stairs_to_hall").text(
      listToString(morningDutyData["stairs_to_hall"])
    );
    $("#morning_side_gate").text(listToString(morningDutyData["side_gate"]));
    $("#morning_lift").text(listToString(morningDutyData["lift"]));
    $("#morning_morning_patrol").text(
      listToString(morningDutyData["morning_patrol"])
    );
  ...
  if (
    checkTimeValid("09:10:00", "11:30:00") &&
    today(String($("#start").val()))
  ) {
